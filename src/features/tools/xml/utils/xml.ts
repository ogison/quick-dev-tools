...
export const formatXml = (xmlString: string, options: XmlFormatOptions = DEFAULT_XML_OPTIONS): string => {
  try {
    ...
    throw new Error('XML formatting failed: ' + (error instanceof Error ? error.message : String(error)));
  }
};
...
export const extractXmlInfo = (xmlString: string): {
  encoding?: string;
  version?: string;
  rootElement?: string;
  namespaces: string[];
