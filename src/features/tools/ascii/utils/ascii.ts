...
export const generateTextArt = (
  text: string,
  options: AsciiOptions = DEFAULT_ASCII_OPTIONS
): string => {
  if (!text.trim()) {return '';}
...
export const generateBorder = (
  ...
  const body = lines.map((line) => border.v + ' ' + line.padEnd(maxLength) + ' ' + border.v);
...
export const getPatternList = (): Array<{
  name: string;
  value: 'box' | 'diamond' | 'triangle';
}> => {
  return [
    { name: 'ボックス', value: 'box' },
